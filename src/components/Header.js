  ...
  return (
    <div className="absolute w-full bg-gradient-to-b from-black px-8 py-3 z-30 flex justify-between">
      ...
      {user && (
        ...
        {gptSearch && <div>
          <select className="bg-purple-500 mx-2 py-1 px-2 my-1" onChange={handleSelectionOfLang}>
          {SUPPORTED_LANG.map(lang=><option key={lang.identifier} value={lang.identifier}>{lang.name}</option>)}
            
          </select>
        </div>}
        <button className="mx-2 px-4 bg-green-300 h-8" onClick={()=>handleGptSearch()}>{gptSearch?"HomePage":"GPT Search"}</button>
